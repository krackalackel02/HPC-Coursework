/**
 * @file SCGAnalyticalTest.cpp
 * @author Paul Kallarackel (paulkallarackel@gmail.com)
 * @brief Contains unit-test for analytical sinusoidal function
 * @version 0.1
 * @date 2024-03-21
 * 
...
BOOST_AUTO_TEST_CASE(SolverCG_SOLVE_AnalayticalTest)
{
    ...
    prl::gridData* GRID= ldc->getGRID(); /// Extract grid data for local error calcs
    ...
    int size = (Chunkx+2)*(Chunky+2); /// size of local vectors
    double *analytical = new double[size]();
    double *solution = new double[size]();
    double *locV = new double[size]();
    const double k = 2.0 / Lx; // frequency to allow BC to be imposed
    ...
    for (int i = start[0]; i <= stop[0]; ++i)
    {
        for (int j = start[1]; j <= stop[1]; ++j)
        {
                ...
                x = i*dx;
                y = (Ny-1-j)*dy;
                /// Analytical sinusoidal function
                locV[idx]= M_PI * M_PI * (k * k + l * l) * sin(M_PI * k * x) * sin(M_PI * l * y );
                analytical[idx] = sin(M_PI * k * x) * sin(M_PI * l * y);
        }
    }
    solver->Solve(locV,solution,GRID); // solve for analytucal solution

    /// locally calc eror rand sum reduce
    double locError = 0.0;
    double ERR = 0.0;
    for (int i = start[0]; i <= stop[0]; ++i)
    {
    ...
    ERR = sqrt(ERR);
    if(world_rank==0)BOOST_CHECK(myTest::compare(ERR,0.001));
